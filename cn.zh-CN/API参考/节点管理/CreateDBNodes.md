...
|DBNode.N.ZoneId|String|否|新增节点的可用区，需要与原有节点保持一致。N为从1开始的整数，最大值=16-当前节点数。DBNode.N.ZoneId和DBNode.N.TargetClass，至少填一个。可用区ID可以通过接口[DescribeRegions](cn.zh-CN/API参考/地域/DescribeRegions.md#)查询。 **说明：** 当前暂不支持一次增加多个节点。

 |
|DBNode.N.TargetClass|String|否|新增节点的规格，需要与原有节点保持一致。N为从1开始的整数，最大值=16-当前节点数。DBNode.N.ZoneId和DBNode.N.TargetClass，至少填一个。规格详情请参见[规格与定价](../../../../cn.zh-CN/产品简介/规格与定价.md#)。 **说明：** 当前暂不支持一次增加多个节点。
