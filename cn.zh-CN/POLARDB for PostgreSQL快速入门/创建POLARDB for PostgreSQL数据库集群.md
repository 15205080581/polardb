# 创建POLARDB for PostgreSQL数据库集群 {#task_nhk_4qg_tdb .task}
...
5.  设置如下参数。 
 ...
 |
    |节点规格|按需选择。所有POLARDB节点均为独享型，性能稳定可靠。关于各规格的具体信息，请参见[规格与定价](../cn.zh-CN/产品简介/规格与定价.md#)。|
 ...
 |
    |存储费用|无需选择。系统会根据实际数据使用量按小时计费，详情请参见[产品价格](../cn.zh-CN/产品简介/规格与定价.md#)。 **说明：** 创建集群时无需选择存储容量，存储容量随数据量的增减而自动弹性伸缩。
